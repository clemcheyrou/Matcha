...
export const useNotification = () => {
  ...
  useEffect(() => {
    ...
    const fetchNotifications = async () => {
        try {
            const response = await fetch(`${process.env.REACT_APP_API_URL}/api/notification`, {credentials: 'include'});
  ...
  const markAsRead = async (notificationId: any) => {
    try {
        const response = await fetch(`${process.env.REACT_APP_API_URL}/api/notification/${notificationId}/read`, {
            method: 'PATCH',
            credentials: 'include',
        });
  ...
  return { notification, showNotification, handleCloseNotification, notifications, error, loading, markAsRead, handleResponse };
};
