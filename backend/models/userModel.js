...
export const findDefaultUsers = async (userId, pool, filters) => {
    try {
		...
		if (result.rows.length > 1)
		{
			...
			// profilesDistances.forEach(user => {
			// 	console.log(`👤 Utilisateur - ID: ${user.id}, Username: ${user.username}`);
			// 	console.log(`📍 Distance: ${user.distance?.toFixed(2) ?? "N/A"} km`);
			// 	console.log('---');
			// });
...
export const addDistanceScore = async (userId, users, pool) => {
	try {
...
export const addFameRating = async (users, pool) => {
	try {
		const FameUsers = [];
